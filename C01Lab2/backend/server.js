    ...
    try {  
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        if (!allNotes) {
          return res
            .status(404)
            .json({ error: "Unable to find allNotes." });
        }
...
app.delete("/deleteNote/:noteId", express.json(), async (req, res) => {
    try {
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        res.json({ response: "Successful request" });
      });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
    ...
    try {  
      ...
      jwt.verify(token, "secret-key", async (err, decoded) => {
        ...
        const updateResult = await collection.updateOne(
          { _id: new ObjectId(noteId) }, // 查询条件，用于定位特定的 note
          { $set: { title, content } }
        );
        ...
        res.json({ response: "Successful update" });
      });
    } catch (error) {
      res.status(500).json({ error: "General server errors" });
    }
  });
